        ...
        try:
            if AUTOPOST_DATA.get("enabled", True):
                ...
                for post in AUTOPOST_DATA.get("posts", []):
                    ...
                    target_groups = post.get("target_groups") or []
                    post_groups = [int(x) for x in target_groups if str(x).lstrip("-").isdigit()] if target_groups else list(groups)

                    for chat_id in post_groups:
                        key = str(chat_id)
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    paused = [g for g in groups if not g.get("active", True)]
    posts = AUTOPOST_DATA.get("posts", []) or []
    status = "🟢 ON" if AUTOPOST_DATA.get("enabled", True) else "🔴 OFF"
    await update.message.reply_text(
        "📊 RJ BOT STATUS\n\n"
        ...
        f"⏸️ Paused Groups: {len(paused)}\n"
        f"📋 Saved Posts: {len(posts)}\n\n"
        "💾 Storage: Firebase + local backup"
    )


async def pausegroup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _admin_only(update):
        return
    if not context.args:
        await update.message.reply_text("ব্যবহার: /pausegroup 2\nআবার চালু করতে: /pausegroup 2")
        return
    raw = context.args[0]
    chat_id = group_id_from_index(raw) if raw.isdigit() and not raw.startswith("-") else None
    if chat_id is None:
        try:
            ...
            chat_id = None
    if chat_id is None:
        await update.message.reply_text("❌ Group পাওয়া যায়নি। /groups দেখুন।")
        return
    groups = get_saved_groups()
    for g in groups:
        if int(g["chat_id"]) == chat_id:
            g["active"] = not bool(g.get("active", True))
            save_autopost_data()
            state = "🟢 Active" if g["active"] else "⏸️ Paused"
            await update.message.reply_text(f"✅ {g.get('title','Group')} এখন {state}।\n\n{group_list_text()}")
            return
    await update.message.reply_text("❌ Group পাওয়া যায়নি। /groups দেখুন।")


async def renamegroup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _admin_only(update):
        return
    ...
    if not new_name:
        await update.message.reply_text("❌ নতুন নাম দিন।")
        return
    for g in get_saved_groups():
        if int(g["chat_id"]) == chat_id:
            g["title"] = new_name
            save_autopost_data()
            await update.message.reply_text(f"✅ Group-এর নাম এখন: {new_name}\n\n{group_list_text()}")
            return
    await update.message.reply_text("❌ Group পাওয়া যায়নি। /groups দেখুন।")
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("ব্যবহার: /broadcast আপনার মেসেজ\nঅথবা কোনো text/photo-তে reply করে /broadcast")
        return
    groups = active_group_ids()
    if not groups:
        await update.message.reply_text("❌ কোনো Active Group নেই। /addgroup দিয়ে Group যোগ করুন।")
        return
    ok = 0
    fail = 0
    for chat_id in groups:
        try:
            ...
            if replied and replied.photo and not text:
                await context.bot.send_photo(chat_id=chat_id, photo=replied.photo[-1].file_id, caption=(replied.caption or "")[:1024] or None)
            else:
                await context.bot.send_message(chat_id=chat_id, text=text[:4096], disable_web_page_preview=True)
            ok += 1
        except Exception as e:
            fail += 1
            logger.error("Broadcast failed | group=%s | %s", chat_id, e)
...
async def autopost_command(
    ...
    if not args:
        await update.message.reply_text(
            ...
            "/autopost addphoto 8:30 PM Caption - সব Active Group-এ photo post\n"
            "/autopost addgroup 1 8:30 PM পোস্ট - শুধু Group 1\n"
            "/autopost addphotogroup 1 8:30 PM Caption - শুধু Group 1-এ photo\n"
    ...
    if action == "addgroup":
        if len(args) < 4:
            await update.message.reply_text("❌ ব্যবহার: /autopost addgroup 1 8:30 PM Group 1-এর পোস্ট")
            return
        group_ref = args[1]
        group_id = group_id_from_index(group_ref) if group_ref.isdigit() and not group_ref.startswith("-") else None
        if group_id is None:
            try:
                group_id = int(group_ref)
            except Exception:
                group_id = None
        if group_id is None or not any(int(g["chat_id"]) == group_id for g in get_saved_groups()):
            await update.message.reply_text("❌ Group পাওয়া যায়নি। /groups দেখুন।")
            return
        time_arg_count = 3
        if len(args) >= 4 and args[3].upper() in ("AM", "PM"):
            time_text = f"{args[2]} {args[3]}"
            time_arg_count = 4
        else:
            time_text = args[2]
        hhmm = parse_ampm_time(time_text)
        if not hhmm:
            await update.message.reply_text("❌ সময় ঠিক নয়। উদাহরণ: 8:30 PM বা 20:30")
            return
        ...
        post = find_autopost(post_id)
        post["target_groups"] = [group_id]
        save_autopost_data()
        await update.message.reply_text(f"✅ শুধু Group {group_ref}-এর জন্য Auto Post যোগ হয়েছে।\n🆔 ID: {post_id}\n⏰ {display_time(hhmm)}")
        return

    if action == "addphotogroup":
        if len(args) < 3:
            await update.message.reply_text("❌ ব্যবহার: photo-তে reply করে /autopost addphotogroup 1 8:30 PM Caption")
            return
        group_ref = args[1]
        group_id = group_id_from_index(group_ref) if group_ref.isdigit() and not group_ref.startswith("-") else None
        if group_id is None:
        ...
        hhmm = parse_ampm_time(time_text)
        replied = update.message.reply_to_message
        if not hhmm or not replied or not replied.photo:
            await update.message.reply_text("❌ photo-তে reply করে সঠিক সময় দিন: /autopost addphotogroup 1 8:30 PM Caption")
            return
        caption = " ".join(args[time_arg_count:]).strip() or (replied.caption or "")
        post_id = add_autopost(hhmm, "photo", text=caption, photo_file_id=replied.photo[-1].file_id)
        post = find_autopost(post_id)
        post["target_groups"] = [group_id]
        save_autopost_data()
        await update.message.reply_text(f"✅ শুধু Group {group_ref}-এর জন্য Photo Auto Post যোগ হয়েছে।\n🆔 ID: {post_id}\n⏰ {display_time(hhmm)}")
        return
...
async def help_command(
    ...
    await update.message.reply_text(
        ...
        "ℹ️ /about\n\n"
        "👥 GROUP CONTROL (Admin)\n"
        "/addgroup - বর্তমান Group যোগ\n"
        "/groups - Group list\n"
        "/delgroup 2 - Group delete\n"
        "/pausegroup 2 - Pause/Resume Group\n"
        "/renamegroup 2 নতুন নাম - নাম বদল\n"
        "/status - Bot/Group status\n"
        "/broadcast মেসেজ - সব Active Group-এ পাঠান\n\n"
        "📅 AUTO POST\n"
        "/autopost on / off\n"
        "/autopost list\n"
        "/autopost add 8:30 PM পোস্ট - সব Active Group\n"
        "/autopost addgroup 1 8:30 PM পোস্ট - শুধু Group 1\n"
        "/autopost addphoto 8:30 PM Caption - সব Active Group\n"
        "/autopost addphotogroup 1 8:30 PM Caption - শুধু Group 1\n"
        "/autopost delete 1 - Delete post"
    )
...
def main():
    ...
    application.add_handler(CommandHandler("renamegroup", renamegroup_command))
